#!/bin/bash
# закомментировать 1-ю строку, сменить интерфейс, разрешить dhcp
sudo sed -i 's/except-interface=lo/#except-interface=wlan0/g' /etc/dnsmasq.conf 1>/dev/null
sudo sed -i 's/no-dhcp-interface=ens2/no-dhcp-interface=eth0/g' /etc/dnsmasq.conf 1>/dev/null
#sudo sed -i 's/no-dhcp-interface=wlan0/#no-dhcp-interface=wlan0/g' /etc/dnsmasq.conf 1>/dev/null
#
sudo sed -i 's/#interface=ens2/interface=eth0/g' /etc/dnsmasq.conf 1>/dev/null
#
...
#B2=$(cat /etc/dnsmasq.conf | sed -n '/dhcp-range/=' | tail -n1)
#sudo sed -i "${B2}d" /etc/dnsmasq.conf 1>/dev/null
##
...
#A3=$(cat /etc/dnsmasq.conf | sed -n '/dhcp-option=6/=' | tail -n1)
#sudo sed -i "${A3}d" /etc/dnsmasq.conf 1>/dev/null
##
